...
struct DuplicateFinderView: View {
    ...
    private func runScan() async {
        ...
        selection.removeAll()
        lastFailures = []
        groups = await DuplicateFinder.find(in: selectedFolders)
        hasScanned = true
        isScanning = false
    }

    private func cleanSelected() {
        let paths = groups.flatMap(\.files).filter { selection.contains($0.id) }.map(\.path)
        Task {
            lastFailures = TrashService.moveToTrash(paths)
            await runScan()
        }
    }
}
